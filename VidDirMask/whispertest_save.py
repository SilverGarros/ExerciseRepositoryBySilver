...
from app import get_filename_without_extension


def TTSByWhisper(audio_cache):
    whisper_model = whisper.load_model("model/medium.pt")
...
def overlay_silence(audio_file_path, time_intervals):
    ...
    # 输出处理后的音频
    output_path = get_filename_without_extension(audio_file_path) + "_silence.mp3"
...
def replace_silence_with_beep(audio_file_path, time_intervals, beep_path):
    ...
    # 输出处理后的音频
    output_path = get_filename_without_extension(audio_file_path) + "_replece_silence_with_beep.mp3"
    audio_file.export(output_path, format="mp3")
    return output_path
